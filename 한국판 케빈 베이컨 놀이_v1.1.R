...
# 배우별 출연 영화수 컬럼 만들기
numMovies <- aggregate(x = wdf$mcode, by = list(wdf$acode), FUN = length)
colnames(numMovies) <- c('acode', 'movieCnt')

# 배우별 영화수 빈도 확인
table(numMovies$movieCnt, useNA = 'ifany')
...
plot(x = table(movieList$myear), type = 'l', xlab = '제작년도', ylab = '영화편수')

# 영화 목록 데이터로 EDA
movieTbl <- as.data.frame(table(movieList$genre, movieList$formt, useNA = 'ifany'))
movieTbl <- dcast(data = movieTbl, formula = Var1 ~ Var2)
colnames(movieTbl)[1] <- 'genre'
...
wdf1 <- wdf[wdf$movieCnt>=10,]
nrow(wdf1)

# 영화별 배우수 컬럼 생성
nActor <- aggregate(wdf1$actorCode, by=list(wdf1$movieCode), FUN=length)
...
# 전체 데이터에 병합
wdf1 <- merge(wdf1, nActor, by='movieCode', all.x=T)

# 영화별 배우수 빈도 확인
table(wdf1$actorCnt, useNA='ifany')

# 배우수 최빈도 영화 확인
# 출연한 영화수가 5편 이상인 배우만 대상으로 했으므로 원래 데이터와 다름!!
wdf1[wdf1$actorCnt==max(wdf1$actorCnt),]

# 출연한 배우수가 2명 이상인 영화만 남기기
wdf1 <- wdf1[wdf1$actorCnt>=2,]
nrow(wdf1)

# 10편 이상 출연한 배우들이 출연한 영화 편수 (중복 제거)
length(unique(wdf1$movieCode))

# 컬럼 정렬
wdf1 <- wdf1[,c('movieCode','movieName','year','actorCnt','actorCode','actorName','movieCnt')]
head(wdf1)

# 2000년 이후 영화만 남기기
wdf2 <- wdf1[wdf1$year>=2000,]
nrow(wdf2)


# 영화*배우 행렬 전처리
mawdf <- cbind(wdf2[,c('movieCode','actorCode')], check=rep(1,nrow(wdf2)))
head(mawdf)

# 영화*배우 행렬 생성
maMat <- acast(mawdf, movieCode~actorCode, fill=0)
dim(maMat)
...
for (i in 1:length(ones)) {
  cowdf <- data.frame(from=rownames(actorsMul)[i], to=colnames(actorsMul)[ones[[i]]])
  actorswdf <- rbind(actorswdf, cowdf)
}

actorswdf <- unique(actorswdf)
dim(actorswdf)

# 최단경로 구하기
graph1 <- graph.data.frame(actorswdf, directed=FALSE)
